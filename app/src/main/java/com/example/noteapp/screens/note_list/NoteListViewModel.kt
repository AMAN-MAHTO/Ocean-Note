...
class NoteListViewModel @Inject constructor(databaseClient: DatabaseClient) :ViewModel() {
    ...
    init {
        viewModelScope.launch {
            ...
            databaseClient.addNote(_notes[0])
        }
    }

    private val _selectedNote = MutableStateFlow(_notes[0])
    ...
    fun updateNote(note: Note){
        _notes.find { it.id == note.id }.apply {
            this?.title = note.title
            this?.body = note.body
            this?.updatedDate = "Wed 14, 2024"
        }
        Log.d("NOTES", "updateNote: ${_notes[0]}")
    }
